...
try:
    # This is Python's way of calling the command line. We use it to compile the C++ files.
    subprocess.check_output(["gcc -std=c++1y -c moo_correct.cpp"],stdin=None,stderr=subprocess.STDOUT,shell=True)
except subprocess.CalledProcessError as e:
    # There were compiler errors in moo_correct.cpp. Print out the error message and exit the program.
    print("<p>",e.output,"</p>")
    raise SystemExit

# Change triangle_type to your function name to create the executable file
subprocess.call('g++ -o correct.out test_cases.o moo_correct.o', stdin=None, stdout=None, stderr=None, shell=True)
...
for i in range(num_incorrect):
    ...
    try:
        # This is Python's way of calling the command line. We use it to compile the C++ files.
        subprocess.check_output(['gcc -std=c++1y -c moo_incorrect'+str(i+1)+'.cpp'],stdin=None,stderr=subprocess.STDOUT,shell=True)
    except subprocess.CalledProcessError as e:
        # There were compiler errors in test_cases.cpp. Print out the error message and exit the program.
        print("<p>",e.output,"</p>")
        raise SystemExit
    #  Change triangle_type to your function name
    subprocess.call('g++ -o incorrect'+str(i+1)+'.out test_cases.o moo_incorrect'+str(i+1)+'.o', stdin=None, stdout=None, stderr=None, shell=True)
